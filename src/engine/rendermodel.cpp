...
float modelradius(int index) //actually this should not be here
{
 model*m = loadmodel(mapmodelname(index)); // lol
 if(!m)return 0.0;
 return m->rejectradius;
}
...
static const char * const mmprefix = ""; // so now we can use any directory for our map model
...
int mapmodelindex(const char*model)
{
 string fullpath;
 formatstring(fullpath,"%s%s",mmprefix,model);
 loopv(mapmodels)
  {
  if(!strcmp(fullpath,mapmodels[i].name))
   {
   return i;
   }
  }
...
ICOMMAND(mapmodelindex,"s",(char*s),{intret(mapmodelindex(s));});
