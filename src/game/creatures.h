...
creatureEntity(vec pos,int attr1,int attr2,int attr3,int attr4,int attr5):playerEnt(),controlled(false),vangles(0,0,0),team(0),warned(false),goodattacktime(-1),target(NULL),knownpositions(0)
{
setpos(pos);roll = attr1;type = E_CREATURE;crtype = findone(game::creatureClasses,attr2);}
~creatureEntity(){}
bool avoid();
