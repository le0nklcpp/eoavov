...
namespace ai
{
    ...
    bool route(playerEnt *d, int node, int goal, vector<int> &route, const avoidset &obstacles, int retries)
    {
        ...
        if(d)
        {
            if(retries <= 1 ) loopi(ai::NUMPREVNODES) if(d->prevnodes[i] != node && iswaypoint(d->prevnodes[i]))
            {
                waypoints[d->prevnodes[i]].route = routeid;
                waypoints[d->prevnodes[i]].curscore = -1;
                waypoints[d->prevnodes[i]].estscore = 0;
            }
            if(retries <= 0)
            {
    ...
    void navigate(playerEnt *d)
    {
        ...
        float dist = dropping ? WAYPOINTRADIUS : SIGHTMIN;
    ...
    void seedwaypoints()
    {
        ...
        loopv(entities::getents())
        {
            ...
            switch(e.type)
            {
                case ET_PLAYERSTART: case ENT_TELEPORT: case ENT_ITEM: case ENT_PROP:
                    addwaypoint(e.o);
                    break;
                default:
