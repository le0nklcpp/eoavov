...
extern int lookuptexture(const vec &o);
extern void deletecube(const vec &pos);
...
extern bool mmintersect(char*path, const vec&pos, int yaw, int pitch, int roll, const vec&o, const vec&ray, float maxdist, int mode, float &dist, float scale=1.0);
...
extern float modelradius(int index);
extern const char *mapmodelname(int i);
extern int mapmodelindex(const char*model);
